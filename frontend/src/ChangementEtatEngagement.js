...
const ModificationEngagement = () => {
  ...
  const [nouvel_etat, setNouvelEtat] = useState('');
  ...
  const handleEditSubmit = (event) => {
    ...
    fetch(`http://localhost:8000/api/realisationBudget/changement_etat/${engagementData.id_depense_siig}/`, {
      method: 'PUT',
      ...
      .then(updatedEngagement => {
        setEngagementData(engagementData.map(t => (t.id_depense_siig === updatedEngagement.id_depense_siig ? updatedEngagement : t)));
      })
    ...
    <div className="form-page">
      ...
      <div className="main-content">
        ...
        <div className="form-container">
          <h2 className="div-h2">Changemnent d'Etat de l'engagement</h2>
          ...
          {engagementData && (
            <div className="details-section">
              ...
              {/* Compte */}
                <input 
                  type="text" 
                  value={engagementData.id_compte} 
                  onChange={(e) => setEngagementData({...engagementData, id_compte: e.target.value})} 
                  required hidden
                />
              ...
              {/* Bouton Enregistrer */}
              <div className="button-group">
                <button type="submit" onClick={handleEditSubmit} className="btn save-btn">Enregistrer
                {error && <p className="error-message">{error}</p>}
                {showConfirmation && <ConfirmationOk message="Les modifications ont été enregistrées avec succès !" onClose={handleOk} />}
              </button>
              </div>
            </div>
          )}
