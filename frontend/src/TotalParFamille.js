...
function TotalParFamille() {
  const navigate = useNavigate();
  const [annee, setAnnee] = useState('');
  const [afficherTableau, setAfficherTableau] = useState(false);
  const [resultats, setResultats] = useState([]); // Pour stocker les données de l'API
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/creationArticle/total_par_famille/${annee}/`);
      if (!response.ok) {
    ...
    <div className="dashboard-page">
      ...
      <div className="main-content">
        ...
        <div className="form-container">
          <h2 className="div-h2">Total par Famille</h2>
                ...
                {resultats.length === 0 ? (
                  <tr>
                    <td colSpan="4" style={{ color: 'red', textAlign: 'center' }}>
                      Aucun résultat trouvé pour l'année {annee}.
                    </td>
                  </tr>
                ) : (
...
export default TotalParFamille;
