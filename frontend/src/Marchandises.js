...
function Marchandises() {
  ...
  const fetchMarchandises = () => {
    const url = `http://localhost:8000/api/statistiqueBrutes/gestion_marchandise/?annee=${choixAnnee}&id_compte=${choixCompte}&code_stat=${codeStat}&type_stat=${libelleStat}`;
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (choixAnnee && choixCompte && codeStat && libelleStat) {
